...
Disini, kita mendapatkan nilai `isAuthenticated` dari auth context, kamu bisa [melihat repositori](https://github.com/mustaqimarifin/learn-auth-redirect-nextjs) untuk melihat code lebih lengkap.
...
Potongan code ini menggunakan useEffect di authentication context, untuk memverifikasi token yang biasanya disimpan di localStorage. Jika kamu ingin mengetahu Authentication Context pattern lebih lanjut, saya memiliki [code snippet](http://ekswhyzee.vercel.app/library/auth-context) tentang itu.
...
> I'm open for all suggestions and contributions to improve 🚀. Open a PR on the repository or email me at me@ekswhyzee.vercel.app
...
<CloudinaryImg
  mdx
  publicId='mstqmarfn/blogs/nextjs-redirect-no-flashing/blocked-unhandled_m4chdj'
...
<CloudinaryImg
  mdx
  publicId='mstqmarfn/blogs/nextjs-redirect-no-flashing/blocked_nbwdvm'
  alt='blocked'
  width={1440}
  height={852}
/>
...
<CloudinaryImg
  mdx
  publicId='mstqmarfn/blogs/nextjs-redirect-no-flashing/blocked-token_lhresk'
  alt='blocked-token'
  width={1440}
  height={852}
/>
