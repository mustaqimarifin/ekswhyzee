...
<CloudinaryImg
  mdx
  publicId='mstqmarfn/blogs/react-loading-state-pattern/normal_fetch_using_axios_uqgjxy'
...
I added a dismiss button because it doesn't have it by default, you can grab the `DismissableToast` code from [my library](https://ekswhyzee.vercel.app/library/toast#dismissabletoast).
...
<CloudinaryImg
  mdx
  publicId='mstqmarfn/blogs/react-loading-state-pattern/toast-using-swr-fetch_vg8bno'
  alt='Toast using SWR'
  width={800}
  height={514}
/>
...
You can look at the demo source code on [github](https://github.com/mustaqimarifin/react-async-pattern), but keep in mind there is additional promise to delay the loading time.
