...
Here, we are getting the `isAuthenticated` from the Auth Context, you can [see the repository](https://github.com/mustaqimarifin/learn-auth-redirect-nextjs) for more details.
...
This code is using useEffect in the Authentication Context, to verify the token that is usually stored in localStorage. You can check my library to [learn more about Authentication Context pattern](http://ekswhyzee.vercel.app/library/auth-context)
